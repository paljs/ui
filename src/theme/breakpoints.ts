...
function breakpointNext(index: number): BreakPointKeys | null {
	return index < Object.keys(breakpoints).length
		? (Object.keys(breakpoints)[index + 1] as keyof Breakpoint)
		: null;
}
...
const breakpointDown: (name: BreakPointKeys) => CSSType = name => (
	first: any,
	...args: any[]
) => {
...
const breakpointUp: (name: BreakPointKeys) => CSSType = name => (
	first: any,
	...args: any[]
) => {
...
const breakpointBetween: (
	lower: BreakPointKeys,
	upper: BreakPointKeys
) => CSSType = (lower, upper) => (first: any, ...args: any[]) => {
...
const breakpointOnly: (name: BreakPointKeys) => CSSType = name => (
	first: any,
	...args: any[]
) => {
	const min = breakpointMin(name);
