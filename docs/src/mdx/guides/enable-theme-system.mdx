  ...
  return (
    <ThemeProvider
      theme={{
        ...
        ...themes(theme, { dir, fontSize: '2rem', fontMain: 'font family' })
      }}
    >
      <Layout dir={dir} />
    </ThemeProvider>
...
export function themeService(theme, settings) {
  switch (theme) {
    case 'default':
    case 'cosmic':
      return themes(theme, { ...settings });
    case 'corporate':
      return themes(theme, { ...settings, ...customCorporate });
    case 'dark':
      // this will take all default theme variables and over it with your custom variables
      return themes(theme, { ...settings, customDark });
  }
}
